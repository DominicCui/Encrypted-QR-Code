  ...
  static void append8BitBytes(String content, BitArray bits, String encoding)
     ...
     */
			...
			for(int i = 0; i < bytes.length; i++) {
				bin += String.format("%8s", Integer.toBinaryString(bytes[i] & 0xFF)).replace(' ', '0');
            }
			reader.encrypt.process(bin);
			double end = System.nanoTime();
			double cost = (end - start) / 1000000.0f;
			System.out.format("Encryption cost time:" + "%.2f", cost);
			System.out.print(" ms \n");
    ...
    for (byte b : reader.encrypt.result_b) {//reader.encrypt.result or bytes
      bits.appendBits(b, 8);
    }
